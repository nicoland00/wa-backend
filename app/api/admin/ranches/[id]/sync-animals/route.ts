...
import { syncRemoteAnimals } from "@/lib/server/sync";
...
export async function POST(_: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const summary = await syncRemoteAnimals(ranch);
    await logAudit({
      actorUserId: actor.userId,
      actorRole: actor.role,
      action: "admin.ranch.sync_animals",
      target: { type: "ranch", id: ranch._id.toString() },
      after: summary,
    });
    return NextResponse.json({ ok: true, summary });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Animal sync failed";
